...
import { db } from '@/db';
import { citiesTable, organizationsTable, petsTable } from '@/db/schema';
import type { PetFilters } from '@/lib/pets';
import { createClient } from '@/lib/supabase/server';
import type { PaginatedResult, QueryOptions } from '@/lib/types';

export type Pet = Omit<
  typeof petsTable.$inferSelect,
  'image' | 'description' | 'temperament' | 'breed' | 'color'
> & {
  cityName: string;
...
export async function getPets(options?: QueryOptions<PetFilters>): Promise<PaginatedResult<Pet>> {
...
export async function getRandomPets(count: number, filters?: PetFilters): Promise<Pet[]> {
  ...
  if (filters?.organizationId)
    whereConditions.push(eq(petsTable.organizationId, filters.organizationId));
